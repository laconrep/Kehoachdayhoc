...
SQL);
    $classCols = $db->query('PRAGMA table_info(classes)')->fetchAll();
    $hasTeacher = false;
    foreach ($classCols as $col) {
        if (($col['name'] ?? '') === 'teacher_id') { $hasTeacher = true; break; }
    }
    if (!$hasTeacher) {
        $db->exec('ALTER TABLE classes ADD COLUMN teacher_id INTEGER REFERENCES teachers(id)');
    }
    $legacyTeacher = (string)($db->query("SELECT value FROM settings WHERE key = 'teacher_name'")->fetchColumn() ?: '');
    if ($legacyTeacher !== '') {
        $db->prepare('INSERT OR IGNORE INTO teachers(ten) VALUES(?)')->execute([$legacyTeacher]);
        $legacyId = (int)$db->query('SELECT id FROM teachers WHERE ten = ' . $db->quote($legacyTeacher))->fetchColumn();
        if ($legacyId) {
            $db->prepare('UPDATE classes SET teacher_id = ? WHERE teacher_id IS NULL')->execute([$legacyId]);
        }
    }
    $tkbSql = (string)$db->query("SELECT sql FROM sqlite_master WHERE type='table' AND name='tkb_slots'")->fetchColumn();
    if ($tkbSql && str_contains($tkbSql, 'UNIQUE(thu, buoi, tiet_so_trong_buoi)')) {
        $db->exec('PRAGMA foreign_keys = OFF');
        $db->exec("CREATE TABLE tkb_slots_new (
            id INTEGER PRIMARY KEY, class_id INTEGER NOT NULL REFERENCES classes(id) ON DELETE CASCADE,
            thu INTEGER NOT NULL CHECK(thu BETWEEN 2 AND 8), buoi TEXT NOT NULL CHECK(buoi IN ('Sáng','Chiều')),
            tiet_so_trong_buoi INTEGER NOT NULL CHECK(tiet_so_trong_buoi BETWEEN 1 AND 5),
            loai_tiet TEXT NOT NULL DEFAULT 'chinh_khoa' CHECK(loai_tiet IN ('chinh_khoa','chuyen_de'))
        )");
        $db->exec('INSERT INTO tkb_slots_new(id,class_id,thu,buoi,tiet_so_trong_buoi,loai_tiet) SELECT id,class_id,thu,buoi,tiet_so_trong_buoi,loai_tiet FROM tkb_slots');
        $db->exec('DROP TABLE tkb_slots');
        $db->exec('ALTER TABLE tkb_slots_new RENAME TO tkb_slots');
...
function findOrCreateTeacher(string $name): int
{
    $name = trim($name);
    if ($name === '') throw new RuntimeException('Nhập tên giáo viên.');
    db()->prepare('INSERT OR IGNORE INTO teachers(ten) VALUES(?)')->execute([$name]);
    $id = (int)scalar('SELECT id FROM teachers WHERE ten = ?', [$name]);
    if (!$id) throw new RuntimeException('Không lưu được tên giáo viên.');
    return $id;
}
function teacherName(?int $id): string
{
    if (!$id) return '';
    return (string)scalar('SELECT ten FROM teachers WHERE id = ?', [$id]);
}
...
function downloadLbg(int $week, int $teacherId = 0, string $subjectName = ''): never
{
    $week = max(1, min((int)value('weeks'), $week)); $start = getWeekStart($week);
    $sql = 'SELECT b.*, ts.thu,ts.buoi,ts.tiet_so_trong_buoi,c.ten_lop,s.ten_mon,i.tiet_so,i.phan_mon,i.bai_hoc,st.ten AS trang_thai, t.ten AS ten_gv FROM buoi_day b JOIN tkb_slots ts ON ts.id=b.tkb_slot_id JOIN classes c ON c.id=ts.class_id JOIN subjects s ON s.id=c.subject_id LEFT JOIN teachers t ON t.id=c.teacher_id LEFT JOIN ppct_items i ON i.id=b.ppct_item_id JOIN teaching_statuses st ON st.id=b.trang_thai_id WHERE b.tuan_hoc=?';
    $params = [$week];
    if ($teacherId) { $sql .= ' AND c.teacher_id=?'; $params[] = $teacherId; }
    if ($subjectName !== '') { $sql .= ' AND s.ten_mon=?'; $params[] = $subjectName; }
    $records = rows($sql . ' ORDER BY ts.thu, CASE ts.buoi WHEN "Sáng" THEN 0 ELSE 1 END, ts.tiet_so_trong_buoi, t.ten, c.ten_lop', $params);
    $headerTeacher = $teacherId ? teacherName($teacherId) : (count(array_unique(array_filter(array_column($records, 'ten_gv')))) === 1 ? (string)($records[0]['ten_gv'] ?? '') : 'Nhiều giáo viên');
    $grid = [1 => [1 => 'KẾ HOẠCH DẠY HỌC'], 2 => [1 => 'Giáo viên:', 5 => $headerTeacher, 9 => 'Từ ngày:', 10 => $start->format('d/m/Y')], 3 => [1 => 'Tuần:', 5 => 'Tuần ' . $week, 9 => 'Đến ngày:', 10 => $start->modify('+6 days')->format('d/m/Y')], 4 => [1 => 'Thứ, ngày', 3 => 'Buổi', 4 => 'Tiết TKB', 5 => 'Môn học', 6 => 'Lớp', 7 => 'Giáo viên', 8 => 'Tiết PPCT', 9 => 'Tên bài dạy', 10 => 'Ghi chú', 11 => 'Trạng thái']];
    $merges = ['A1:K1','A2:D2','E2:G2','A3:D3','E3:G3']; $row = 5;
    if ($teacherId) {
        $lookup = []; foreach ($records as $r) $lookup[$r['thu'].'|'.$r['buoi'].'|'.$r['tiet_so_trong_buoi']] = $r;
        foreach (DAYS as $day => $dayName) foreach (SESSIONS as $session) { $first = $row; for ($period = 1; $period <= 5; $period++, $row++) { $record = $lookup[$day.'|'.$session.'|'.$period] ?? null; $grid[$row] = [1 => $period === 1 ? ($session === 'Sáng' ? ($day === 8 ? 'CN' : $day) : $start->modify('+' . ($day - 2) . ' days')->format('d/m/Y')) : '', 3 => $period === 1 ? $session : '', 4 => $period]; if ($record) $grid[$row] += [5 => $record['ten_mon'], 6 => $record['ten_lop'], 7 => $record['ten_gv'], 8 => $record['tiet_so'], 9 => $record['bai_hoc'], 10 => $record['ghi_chu'], 11 => $record['trang_thai']]; } $merges[] = 'A' . $first . ':A' . ($row - 1); $merges[] = 'C' . $first . ':C' . ($row - 1); }
    } else {
        foreach ($records as $record) {
            $grid[$row++] = [1 => DAYS[(int)$record['thu']] ?? $record['thu'], 3 => $record['buoi'], 4 => $record['tiet_so_trong_buoi'], 5 => $record['ten_mon'], 6 => $record['ten_lop'], 7 => $record['ten_gv'], 8 => $record['tiet_so'], 9 => $record['bai_hoc'], 10 => $record['ghi_chu'], 11 => $record['trang_thai']];
        }
        if (!$records) { $grid[$row++] = [1 => 'Chưa có buổi dạy']; }
    }
    $sign = $row + 1; $grid[$sign] = [1 => 'Duyệt của TCM', 8 => 'Duyệt của Hiệu trưởng']; $merges[] = 'A'.$sign.':G'.$sign; $merges[] = 'H'.$sign.':K'.$sign;
    $classSql = 'SELECT c.ten_lop,s.ten_mon,t.ten AS ten_gv FROM classes c JOIN subjects s ON s.id=c.subject_id LEFT JOIN teachers t ON t.id=c.teacher_id WHERE 1=1';
    $classParams = [];
    if ($teacherId) { $classSql .= ' AND c.teacher_id=?'; $classParams[] = $teacherId; }
    if ($subjectName !== '') { $classSql .= ' AND s.ten_mon=?'; $classParams[] = $subjectName; }
    $classes = rows($classSql . ' ORDER BY c.ten_lop', $classParams); $grid2 = [1 => [1 => 'Lớp', 2 => 'Môn', 3 => 'Giáo viên']]; foreach ($classes as $i => $class) { $grid2[$i+2] = [1 => $class['ten_lop'], 2 => $class['ten_mon'], 3 => $class['ten_gv']]; }
    ...
    try {
        switch ($action) {
            ...
            case 'add_class':
                ...
                $teacherId = findOrCreateTeacher((string)($_POST['teacher_name'] ?? ''));
                db()->prepare('INSERT INTO classes(ten_lop,khoi,subject_id,teacher_id) VALUES(?,?,?,?)')->execute([$name,$grade,$subjectId,$teacherId]); $classId=(int)db()->lastInsertId(); $cdTrackId=(int)scalar('SELECT id FROM ppct_tracks WHERE subject_id=? AND loai_track=?', [$subjectId,'chuyen_de']); if ($cdTrackId) db()->prepare('INSERT OR IGNORE INTO class_chuyen_de_assignment(class_id,track_id,so_tiet_tuan) VALUES(?,?,1)')->execute([$classId,$cdTrackId]);
                if (!value('teacher_name')) db()->prepare("INSERT INTO settings(key,value) VALUES('teacher_name',?) ON CONFLICT(key) DO UPDATE SET value=excluded.value")->execute([(string)scalar('SELECT ten FROM teachers WHERE id=?', [$teacherId])]);
                flash('success', 'Đã thêm lớp và gán giáo viên vào lịch.'); back('classes');
            case 'assign_teacher':
                $classId = (int)$_POST['class_id']; $teacherId = findOrCreateTeacher((string)($_POST['teacher_name'] ?? ''));
                if (!scalar('SELECT id FROM classes WHERE id=?', [$classId])) throw new RuntimeException('Không tìm thấy lớp.');
                db()->prepare('UPDATE classes SET teacher_id = ? WHERE id = ?')->execute([$teacherId, $classId]);
                flash('success', 'Đã gán giáo viên cho lớp.'); back('classes');
            ...
            case 'save_timetable':
                backup(); $db=db(); $db->beginTransaction(); try {
                    $db->exec('DELETE FROM tkb_slots');
                    $st=$db->prepare('INSERT INTO tkb_slots(class_id,thu,buoi,tiet_so_trong_buoi,loai_tiet) VALUES(?,?,?,?,?)');
                    $seen=[];
                    foreach ($_POST['slot'] ?? [] as $teacherKey=>$days) foreach ($days as $day=>$sessions) foreach ($sessions as $session=>$periods) foreach ($periods as $period=>$selection) {
                        if (!$selection) continue;
                        [$classId,$type] = explode(':', (string)$selection, 2);
                        $classId=(int)$classId;
                        $owner=(int)scalar('SELECT COALESCE(teacher_id,0) FROM classes WHERE id=?', [$classId]);
                        if (!$classId || $owner !== (int)$teacherKey) throw new RuntimeException('Lớp không thuộc giáo viên đang xếp TKB.');
                        $dupKey = $owner.'|'.$day.'|'.$session.'|'.$period;
                        if (isset($seen[$dupKey])) throw new RuntimeException('Một giáo viên không thể gán hai lớp cùng một ô TKB.');
                        $seen[$dupKey]=true;
                        $st->execute([$classId,(int)$day,$session,(int)$period,$type === 'chuyen_de' ? 'chuyen_de':'chinh_khoa']);
                    }
                    $db->commit(); flash('success','Đã lưu thời khóa biểu theo từng giáo viên. Nhấn “Sinh lịch” để cập nhật kế hoạch.');
                } catch(Throwable $e){$db->rollBack();throw $e;} back('classes');
            ...
            case 'update_lesson':
                ...
                flash($status === 'Nghỉ' ? 'warning' : 'success', $message); back('schedule',['week'=>(int)$_POST['week'], 'teacher'=>(int)($_POST['teacher_id'] ?? 0), 'subject'=>trim((string)($_POST['subject_name'] ?? ''))]);
            case 'download_xlsx': downloadLbg((int)$_POST['week'], (int)($_POST['teacher_id'] ?? 0), trim((string)($_POST['subject_name'] ?? '')));
...
<?php endif ?>
<?php elseif ($page === 'classes'): $subjects=rows('SELECT * FROM subjects ORDER BY khoi,ten_mon'); $teachers=rows('SELECT * FROM teachers ORDER BY ten'); $classes=rows('SELECT c.*,s.ten_mon,s.so_tiet_tuan_mac_dinh,t.ten AS ten_gv FROM classes c JOIN subjects s ON s.id=c.subject_id LEFT JOIN teachers t ON t.id=c.teacher_id ORDER BY c.khoi,c.ten_lop'); $slots=rows('SELECT ts.*, c.teacher_id FROM tkb_slots ts JOIN classes c ON c.id=ts.class_id'); $slotMap=[]; foreach($slots as $slot) $slotMap[(int)$slot['teacher_id']][$slot['thu']][$slot['buoi']][$slot['tiet_so_trong_buoi']]=$slot; $classesByTeacher=[]; foreach($classes as $class) $classesByTeacher[(int)$class['teacher_id']][]=$class; $timetableWarnings=timetableWarnings(); ?>
<section class="hero"><div><h1>Lớp học & thời khóa biểu</h1><p>Mỗi giáo viên có TKB riêng. Nhập tên GV, chọn môn, thêm lớp rồi xếp lịch. Hai GV có thể dạy cùng một tiết.</p></div><?php if($stats['slots']): ?><form method="post"><input type="hidden" name="csrf" value="<?=csrf()?>"><input type="hidden" name="action" value="generate_schedule"><button class="btn" type="submit">◷ Sinh lịch giảng dạy</button></form><?php endif ?></section>
...
<div class="grid two"><section class="card"><h2>Thêm lớp phụ trách</h2><?php if(!$subjects): ?><p class="empty">Hãy import PPCT trước để có môn–khối.</p><?php else:?><form method="post" style="margin-top:15px"><input type="hidden" name="csrf" value="<?=csrf()?>"><input type="hidden" name="action" value="add_class"><input type="hidden" name="return_page" value="classes"><div class="form-grid"><div class="field"><label>Tên lớp *</label><input name="class_name" placeholder="Ví dụ: 11A1" required></div><div class="field"><label>Môn học – khối *</label><select name="subject_id"><?php foreach($subjects as $subject): ?><option value="<?=$subject['id']?>"><?=h($subject['ten_mon'])?> · Khối <?=$subject['khoi']?> · <?=$subject['so_tiet_tuan_mac_dinh']?> tiết/tuần</option><?php endforeach ?></select></div><div class="field"><label>Tên giáo viên *</label><input name="teacher_name" list="teacher-names" placeholder="Ví dụ: Nguyễn Văn A" value="<?=h(value('teacher_name'))?>" required></div></div><datalist id="teacher-names"><?php foreach($teachers as $gv): ?><option value="<?=h($gv['ten'])?>"><?php endforeach ?></datalist><div class="action-row"><button class="btn">+ Thêm lớp</button></div></form><?php endif ?></section><section class="card"><h2>Lớp đã khai báo</h2><?php if(!$classes): ?><p class="empty">Chưa có lớp nào.</p><?php else:?><ul class="list"><?php foreach($classes as $class): ?><li><span><strong><?=h($class['ten_lop'])?></strong><span class="mini"><br><?=h($class['ten_mon'])?> · Khối <?=$class['khoi']?> · <?=h($class['ten_gv'] ?: 'Chưa gán GV')?></span></span><form class="inline-form" method="post" style="display:flex;gap:6px;align-items:center"><input type="hidden" name="csrf" value="<?=csrf()?>"><input type="hidden" name="action" value="assign_teacher"><input type="hidden" name="class_id" value="<?=$class['id']?>"><input name="teacher_name" list="teacher-names" value="<?=h((string)$class['ten_gv'])?>" placeholder="Tên GV" style="width:150px"><button class="btn quiet">Gán</button></form><form class="inline-form" method="post" onsubmit="return confirm('Xóa lớp này và dữ liệu lịch liên quan?')"><input type="hidden" name="csrf" value="<?=csrf()?>"><input type="hidden" name="action" value="delete_class"><input type="hidden" name="class_id" value="<?=$class['id']?>"><button class="btn danger">Xóa</button></form></li><?php endforeach ?></ul><?php endif ?></section></div>
...
<section class="card" style="margin-top:18px"><h2>Thời khóa biểu theo giáo viên</h2><p class="subtle">Mỗi GV một khung riêng. Trong một GV, mỗi ô chỉ một lớp. “CĐ” là tiết chuyên đề.</p>
<?php if(!empty($classesByTeacher[0])): ?><div class="notice warning">Có lớp chưa gán giáo viên — gán tên GV ở danh sách lớp trước khi xếp TKB.</div><?php endif ?>
<?php $hasTeacherGrid=false; foreach($teachers as $gv) if(!empty($classesByTeacher[(int)$gv['id']])) { $hasTeacherGrid=true; break; } ?>
<?php if(!$hasTeacherGrid): ?><p class="empty" style="margin-top:12px">Thêm lớp và gán giáo viên để xếp TKB.</p>
<?php else: ?><form method="post"><input type="hidden" name="csrf" value="<?=csrf()?>"><input type="hidden" name="action" value="save_timetable"><input type="hidden" name="return_page" value="classes">
<?php foreach($teachers as $gv): $gvId=(int)$gv['id']; $gvClasses=$classesByTeacher[$gvId]??[]; if(!$gvClasses) continue; ?>
<h3 style="margin:18px 0 8px"><?=h($gv['ten'])?></h3>
<div class="slot-grid"><table><thead><tr><th>Buổi · tiết</th><?php foreach(DAYS as $day):?><th><?=h($day)?></th><?php endforeach?></tr></thead><tbody>
<?php foreach(SESSIONS as $session): for($period=1;$period<=5;$period++): ?><tr><th><?=$session?><br>Tiết <?=$period?></th>
<?php foreach(DAYS as $day=>$name): $selectedSlot=$slotMap[$gvId][$day][$session][$period]??null; ?>
<td><select name="slot[<?=$gvId?>][<?=$day?>][<?=$session?>][<?=$period?>]"><option value="">— Trống —</option>
<?php foreach($gvClasses as $class): ?>
<option value="<?=$class['id']?>:chinh_khoa" <?=($selectedSlot && $selectedSlot['class_id']==$class['id'] && $selectedSlot['loai_tiet']==='chinh_khoa')?'selected':''?>><?=h($class['ten_lop'].' · '.$class['ten_mon'])?></option>
<?php $hasElective=(bool)scalar('SELECT 1 FROM class_chuyen_de_assignment WHERE class_id=?',[$class['id']]); if($hasElective): ?>
<option value="<?=$class['id']?>:chuyen_de" <?=($selectedSlot && $selectedSlot['class_id']==$class['id'] && $selectedSlot['loai_tiet']==='chuyen_de')?'selected':''?>><?=h($class['ten_lop'])?> · CĐ</option>
<?php endif; endforeach ?></select></td>
<?php endforeach ?></tr><?php endfor; endforeach ?>
</tbody></table></div>
<?php endforeach ?>
<div class="action-row"><button class="btn" type="submit">Lưu thời khóa biểu</button></div></form><?php endif ?></section><?php endif ?>
<?php elseif ($page === 'schedule'): $week=max(1,min((int)value('weeks'),(int)($_GET['week']??1))); $filterTeacher=(int)($_GET['teacher']??0); $filterSubject=trim((string)($_GET['subject']??'')); $scheduleSql='SELECT b.*,ts.thu,ts.buoi,ts.tiet_so_trong_buoi,ts.loai_tiet,c.id AS class_id,c.ten_lop,c.khoi,c.subject_id,s.ten_mon,i.tiet_so,i.bai_hoc,i.noi_dung_chi_tiet,st.ten AS trang_thai,gv.ten AS ten_gv FROM buoi_day b JOIN tkb_slots ts ON ts.id=b.tkb_slot_id JOIN classes c ON c.id=ts.class_id JOIN subjects s ON s.id=c.subject_id LEFT JOIN teachers gv ON gv.id=c.teacher_id LEFT JOIN ppct_items i ON i.id=b.ppct_item_id JOIN teaching_statuses st ON st.id=b.trang_thai_id WHERE b.tuan_hoc=?'; $scheduleParams=[$week]; if($filterTeacher){$scheduleSql.=' AND c.teacher_id=?'; $scheduleParams[]=$filterTeacher;} if($filterSubject!==''){$scheduleSql.=' AND s.ten_mon=?'; $scheduleParams[]=$filterSubject;} $scheduleSql.=' ORDER BY ts.thu,CASE ts.buoi WHEN "Sáng" THEN 0 ELSE 1 END,ts.tiet_so_trong_buoi'; $records=rows($scheduleSql,$scheduleParams); $teacherOptions=rows('SELECT * FROM teachers ORDER BY ten'); $subjectOptions=rows('SELECT DISTINCT ten_mon FROM subjects ORDER BY ten_mon'); $ppctItemsByClass=[]; foreach(rows("SELECT c.id AS class_id,i.id,i.tiet_so,i.bai_hoc,t.loai_track FROM classes c JOIN ppct_tracks t ON t.subject_id=c.subject_id JOIN ppct_items i ON i.track_id=t.id LEFT JOIN class_chuyen_de_assignment a ON a.class_id=c.id AND a.track_id=t.id WHERE t.loai_track='chinh_khoa' OR a.id IS NOT NULL ORDER BY c.id,i.tiet_so,i.id") as $ppctItem) $ppctItemsByClass[$ppctItem['class_id']][]=$ppctItem; $coreSequences=[]; foreach($records as $record) if($record['loai_tiet']==='chinh_khoa') $coreSequences[$record['khoi'].':'.$record['subject_id']][$record['class_id']][]=$record['ppct_item_id'] ?: 0; $syncWarnings=[]; foreach($coreSequences as $group=>$sequences) if(count($sequences)>1 && count(array_unique(array_map(static fn($sequence)=>implode(',',$sequence),$sequences)))>1) { $classNames=[]; foreach($records as $record) if($record['loai_tiet']==='chinh_khoa' && $record['khoi'].':'.$record['subject_id']===$group) $classNames[$record['class_id']]=$record['ten_lop']; $syncWarnings[]=implode(', ',$classNames); } ?>
<section class="hero"><div><h1>Lịch giảng dạy</h1><p>Tuần <?=$week?> · <?=h(getWeekStart($week)->format('d/m/Y'))?> — <?=h(getWeekStart($week)->modify('+6 days')->format('d/m/Y'))?></p></div><?php if($records):?><form method="post"><input type="hidden" name="csrf" value="<?=csrf()?>"><input type="hidden" name="action" value="download_xlsx"><input type="hidden" name="week" value="<?=$week?>"><input type="hidden" name="teacher_id" value="<?=$filterTeacher?>"><input type="hidden" name="subject_name" value="<?=h($filterSubject)?>"><button class="btn secondary">⇩ Xuất LBG .xlsx</button></form><?php endif?></section>
<form method="get" class="card" style="margin-bottom:14px;padding:14px 18px"><input type="hidden" name="page" value="schedule"><input type="hidden" name="week" value="<?=$week?>"><div class="form-grid"><div class="field"><label>Giáo viên</label><select name="teacher" onchange="this.form.submit()"><option value="0">Tất cả giáo viên</option><?php foreach($teacherOptions as $gv):?><option value="<?=$gv['id']?>" <?=$filterTeacher===(int)$gv['id']?'selected':''?>><?=h($gv['ten'])?></option><?php endforeach?></select></div><div class="field"><label>Môn học</label><select name="subject" onchange="this.form.submit()"><option value="">Tất cả môn</option><?php foreach($subjectOptions as $mon):?><option value="<?=h($mon['ten_mon'])?>" <?=$filterSubject===$mon['ten_mon']?'selected':''?>><?=h($mon['ten_mon'])?></option><?php endforeach?></select></div></div></form>
...
<div class="card"><div class="action-row" style="margin-top:0"><a class="btn quiet <?=$week<=1?'hide-mobile':''?>" href="?page=schedule&week=<?=$week-1?>&teacher=<?=$filterTeacher?>&subject=<?=h(urlencode($filterSubject))?>">← Tuần trước</a><form method="get" class="inline-form"><input type="hidden" name="page" value="schedule"><input type="hidden" name="teacher" value="<?=$filterTeacher?>"><input type="hidden" name="subject" value="<?=h($filterSubject)?>"><select name="week" onchange="this.form.submit()" style="width:auto"><?php for($w=1;$w<=(int)value('weeks');$w++):?><option value="<?=$w?>" <?=$w===$week?'selected':''?>>Tuần <?=$w?></option><?php endfor?></select></form><a class="btn quiet <?=$week>=(int)value('weeks')?'hide-mobile':''?>" href="?page=schedule&week=<?=$week+1?>&teacher=<?=$filterTeacher?>&subject=<?=h(urlencode($filterSubject))?>">Tuần sau →</a></div><?php if(!$records):?><div class="empty"><span class="empty-icon">◷</span><strong>Chưa có lịch để hiển thị</strong><p>Hãy khai báo lớp, thời khóa biểu, rồi nhấn “Sinh lịch giảng dạy”.</p><a class="btn" href="?page=classes">Đi tới Lớp & TKB</a></div><?php else:?><div style="margin-top:12px"><?php foreach($records as $record): ?><div class="schedule-row"><div><strong><?=h(DAYS[$record['thu']])?></strong><small class="mini"><?=h(date('d/m',strtotime($record['ngay_duong_lich'])))?></small></div><div><strong><?=h($record['buoi'])?></strong><small class="mini">Tiết TKB <?=$record['tiet_so_trong_buoi']?></small></div><div><span class="badge core"><?=h($record['ten_lop'])?></span><small class="mini" style="display:block;margin-top:5px"><?=h($record['ten_mon'])?> · <?=h($record['ten_gv'] ?: 'Chưa gán GV')?></small></div><div class="schedule-title"><?php if($record['tiet_so']): ?>Tiết PPCT <?=$record['tiet_so']?> · <?=h($record['bai_hoc'])?><small><?=h($record['noi_dung_chi_tiet'] ?: 'Chưa có nội dung chi tiết')?></small><?php else:?><span class="badge warn">Chưa có PPCT</span><small>Kiểm tra số tiết/tuần hoặc bố trí tiết dạy bù.</small><?php endif?></div><form method="post"><input type="hidden" name="csrf" value="<?=csrf()?>"><input type="hidden" name="action" value="update_lesson"><input type="hidden" name="return_page" value="schedule"><input type="hidden" name="lesson_id" value="<?=$record['id']?>"><input type="hidden" name="week" value="<?=$week?>"><input type="hidden" name="teacher_id" value="<?=$filterTeacher?>"><input type="hidden" name="subject_name" value="<?=h($filterSubject)?>"><select name="ppct_item_id" aria-label="Tiết PPCT" title="Chọn tiết PPCT, đặc biệt khi dạy bù"><option value="">— Chưa gán PPCT —</option><?php foreach($ppctItemsByClass[$record['class_id']]??[] as $option): ?><option value="<?=$option['id']?>" <?=$record['ppct_item_id']==$option['id']?'selected':''?>>PPCT <?=$option['tiet_so']?> · <?=h(mb_strimwidth($option['bai_hoc'],0,38,'…','UTF-8'))?><?= $option['loai_track']==='chuyen_de'?' (CĐ)':'' ?></option><?php endforeach ?></select><select name="status" aria-label="Trạng thái" style="margin-top:5px"><?php foreach(STATUS_NAMES as $status):?><option <?=$record['trang_thai']===$status?'selected':''?>><?=h($status)?></option><?php endforeach?></select><input name="note" value="<?=h($record['ghi_chu'])?>" placeholder="Ghi chú" style="margin-top:5px"><button class="btn quiet" style="margin-top:5px;width:100%">Lưu</button></form></div><?php endforeach ?></div><?php endif?></div>
<?php elseif($page === 'progress'): $filterTeacher=(int)($_GET['teacher']??0); $filterSubject=trim((string)($_GET['subject']??'')); $progressSql='SELECT c.*,s.ten_mon,gv.ten AS ten_gv FROM classes c JOIN subjects s ON s.id=c.subject_id LEFT JOIN teachers gv ON gv.id=c.teacher_id WHERE 1=1'; $progressParams=[]; if($filterTeacher){$progressSql.=' AND c.teacher_id=?'; $progressParams[]=$filterTeacher;} if($filterSubject!==''){$progressSql.=' AND s.ten_mon=?'; $progressParams[]=$filterSubject;} $classes=rows($progressSql.' ORDER BY c.khoi,c.ten_lop',$progressParams); $teacherOptions=rows('SELECT * FROM teachers ORDER BY ten'); $subjectOptions=rows('SELECT DISTINCT ten_mon FROM subjects ORDER BY ten_mon'); $selectedClass=(int)($_GET['class']??($classes[0]['id']??0)); $lessons=$selectedClass?rows('SELECT b.*,i.tiet_so,i.bai_hoc,st.ten AS trang_thai FROM buoi_day b JOIN tkb_slots ts ON ts.id=b.tkb_slot_id LEFT JOIN ppct_items i ON i.id=b.ppct_item_id JOIN teaching_statuses st ON st.id=b.trang_thai_id WHERE ts.class_id=? ORDER BY b.tuan_hoc,ts.thu,ts.buoi,ts.tiet_so_trong_buoi',[$selectedClass]):[]; $done=count(array_filter($lessons,fn($x)=>$x['trang_thai']!=='Nghỉ'&&$x['tiet_so'])); $total=count($lessons); ?>
<section class="hero"><div><h1>Tiến độ theo lớp</h1><p>Lọc theo giáo viên hoặc môn học, rồi xem tiến độ từng lớp.</p></div></section><section class="card"><?php if(!$classes && !$filterTeacher && $filterSubject===''):?><div class="empty">Chưa có lớp để theo dõi.</div><?php else:?><form method="get"><input type="hidden" name="page" value="progress"><div class="form-grid"><div class="field"><label>Giáo viên</label><select name="teacher" onchange="this.form.submit()"><option value="0">Tất cả</option><?php foreach($teacherOptions as $gv):?><option value="<?=$gv['id']?>" <?=$filterTeacher===(int)$gv['id']?'selected':''?>><?=h($gv['ten'])?></option><?php endforeach?></select></div><div class="field"><label>Môn học</label><select name="subject" onchange="this.form.submit()"><option value="">Tất cả</option><?php foreach($subjectOptions as $mon):?><option value="<?=h($mon['ten_mon'])?>" <?=$filterSubject===$mon['ten_mon']?'selected':''?>><?=h($mon['ten_mon'])?></option><?php endforeach?></select></div><div class="field"><label>Chọn lớp</label><select name="class" onchange="this.form.submit()"><?php foreach($classes as $c):?><option value="<?=$c['id']?>" <?=$c['id']==$selectedClass?'selected':''?>><?=h($c['ten_lop'].' · '.$c['ten_mon'].' · '.($c['ten_gv']?:'Chưa gán GV'))?></option><?php endforeach?></select></div></div></form><div class="grid three" style="margin:22px 0"><div><strong style="font-size:25px"><?=$done?></strong><span class="mini"> tiết đã lên lịch</span></div><div><strong style="font-size:25px"><?=$total?></strong><span class="mini"> buổi trong năm</span></div><div><strong style="font-size:25px"><?= $total?round($done/$total*100):0 ?>%</strong><span class="mini"> tiến độ kế hoạch</span></div></div><div class="progress-bar"><span style="width:<?=$total?min(100,$done/$total*100):0?>%"></span></div><div class="table-wrap" style="margin-top:20px"><table class="table"><thead><tr><th>Tuần</th><th>Ngày</th><th>Tiết PPCT</th><th>Bài học</th><th>Trạng thái</th></tr></thead><tbody><?php foreach($lessons as $lesson):?><tr><td>Tuần <?=$lesson['tuan_hoc']?></td><td><?=h(date('d/m/Y',strtotime($lesson['ngay_duong_lich'])))?></td><td><?=$lesson['tiet_so']?:'—'?></td><td><?=h($lesson['bai_hoc']?:'Chưa gán PPCT')?></td><td><span class="badge <?=$lesson['trang_thai']==='Nghỉ'?'warn':'ok'?>"><?=h($lesson['trang_thai'])?></span></td></tr><?php endforeach?></tbody></table></div><?php endif?></section>
<?php elseif($page === 'export'): $teacherOptions=rows('SELECT * FROM teachers ORDER BY ten'); $subjectOptions=rows('SELECT DISTINCT ten_mon FROM subjects ORDER BY ten_mon'); ?>
<section class="hero"><div><h1>Xuất báo cáo</h1><p>Tải Lịch báo giảng tuần theo giáo viên hoặc môn học. Tên GV trên file lấy từ lớp đã gán.</p></div></section><section class="card"><h2>Lịch báo giảng tuần</h2><p class="subtle">Chọn tuần, có thể lọc theo giáo viên và môn rồi tải Excel.</p><form method="post" style="margin-top:18px"><input type="hidden" name="csrf" value="<?=csrf()?>"><input type="hidden" name="action" value="download_xlsx"><div class="form-grid"><div class="field"><label>Chọn tuần</label><select name="week"><?php for($w=1;$w<=(int)value('weeks');$w++):?><option value="<?=$w?>">Tuần <?=$w?> · <?=h(getWeekStart($w)->format('d/m'))?></option><?php endfor?></select></div><div class="field"><label>Giáo viên</label><select name="teacher_id"><option value="0">Tất cả</option><?php foreach($teacherOptions as $gv):?><option value="<?=$gv['id']?>"><?=h($gv['ten'])?></option><?php endforeach?></select></div><div class="field"><label>Môn học</label><select name="subject_name"><option value="">Tất cả</option><?php foreach($subjectOptions as $mon):?><option value="<?=h($mon['ten_mon'])?>"><?=h($mon['ten_mon'])?></option><?php endforeach?></select></div></div><div class="action-row"><button class="btn">⇩ Tải Lịch báo giảng .xlsx</button></div></form></section><section class="card" style="margin-top:18px"><h3>Ghi chú</h3><ul class="subtle"><li>Mỗi lần tải được tạo mới từ dữ liệu mới nhất trong hệ thống.</li><li>Trạng thái “Nghỉ” được giữ lại và tiết PPCT trống để bạn chủ động bố trí dạy bù.</li><li>Hệ thống không gửi dữ liệu giảng dạy của bạn tới dịch vụ bên thứ ba.</li></ul></section>
